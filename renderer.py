...
class OctreeRender_trilinear_fast:
    def __init__(self, tensorf, chunk=4096, ndc_ray=False, white_bg=True, device='cuda'):
    ...
    def render(self, rays, white_bg=None, is_train=None):
        rgb_maps, rgbs, depth_maps, disparity_maps, weights, loss_dists, num_samples = [], [], [], [], [], [], []
        ...
        for chunk_idx in range(N_rays_all // self.chunk + int(N_rays_all % self.chunk > 0)):
            rays_chunk = rays[chunk_idx * self.chunk:(chunk_idx + 1) * self.chunk].to(self.device)
        
            outputs = self.tensorf(
                rays_chunk, 
                self.mask, 
                is_train = is_train, 
                white_bg = white_bg, 
            ...
            rgb_maps.append(outputs['rgb_map'])
            rgbs.append(outputs['rgb'])
            depth_maps.append(outputs['depth_map'])
            disparity_maps.append(outputs['disparity_map'])
            num_samples.append(float(outputs['num_valid_samples']))
            loss_dists.append(outputs['loss_dist'])
            weights.append(outputs['weight'])

        output = {
            'rgb_map': torch.cat(rgb_maps),
            'rgb': torch.cat(rgbs),
            'depth': torch.cat(depth_maps),
            'disparity': torch.cat(disparity_maps),
            'weight': torch.cat(weights),
            'loss_dist': torch.cat(loss_dists).sum(),
...
def PSNRs_calculate(args, tensorf, dataset, renderer, chunk=4096, N_samples=-1,
    ...
    for idx, samples in tqdm(enumerate(dataset.all_rays[0::img_eval_interval]), file=sys.stdout):
        ...
        # Get rendered result
        output = renderer.render(rays)
        
        # Get rgb_map and depth_map
        rgb_map = output['rgb_map']
        depth_map = output['disparity']
