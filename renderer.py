...
@torch.no_grad()
def PSNRs_calculate(dataset,tensorf, args, renderer, step, total_freq_reg_step, N_vis=5, N_samples=-1,
...
def save_rendered_image_per_train(train_dataset, test_dataset, tensorf, renderer, current_step, total_train_step, logs, step_size, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    if savePath is not None:
        ...
        ax[0][0].set_title(f"Predicted train Image: {current_step:03d}")

        ax[0][1].imshow(train_depth_map)
        ax[0][1].set_title(f"Train Image with Depth Map: {current_step:03d}")

        ax[1][0].imshow(test_rgb_map)
        ax[1][0].set_title(f"Predicted test Image: {current_step:03d}")

        ax[1][1].imshow(test_depth_map)
        ax[1][1].set_title(f"Test Image with Depth Map: {current_step:03d}")
        ...
        ax[2][1].set_title(f"Train test psnr Plot: {current_step:03d}")
