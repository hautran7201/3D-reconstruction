...
import time
import math
from .nerf_math import *
from .mip import *


def positional_encoding(positions, freqs, step, T=None, Lmax=None, is_pos=False, append_identity=True):
    ...
    if step>-1:
        pts[:, int(step / T * Lmax) + 3:] = 0.
    return pts

def positional_encoding(positions, freqs, append_identity=True):
    freq_bands = (2**torch.arange(freqs).float()).to(positions.device)  # (F,)
    pts = (positions[..., None] * freq_bands).reshape(
        positions.shape[:-1] + (freqs * positions.shape[-1], ))  # (..., DF)
    pts = torch.cat([positions, torch.sin(pts), torch.cos(pts)], dim=-1)
...
class MLPRender_PE(torch.nn.Module):
    def __init__(self,inChanel, viewpe=6, pospe=6, featureC=128):
        ...
        self.in_mlpC = (3+2*viewpe*3) + (3+2*pospe*3) + 3 + inChanel #
        self.viewpe = viewpe
        self.pospe = pospe
...
class NeRF_v3_2(torch.nn.Module):
    '''Based on NeRF_v3, move positional embedding out'''

    def get_activation(self, act):
        if act.lower() == 'relu':
            func = torch.nn.ReLU(inplace=True)
        elif act.lower() == 'lrelu':
            func = torch.nn.LeakyReLU(inplace=True)
        elif act.lower() == 'none':
            func = None
        else:
    ...
    def __init__(self, inChanel, pospe=6, viewpe=6, feape=6, 
        ...
        # get network width
        if layerwise_netwidths:
            Ws = [int(x) for x in layerwise_netwidths.split(',')] + [3]
            print('Layer-wise widths are given. Overwrite args.netwidth')
        else:
            Ws = [W] * (D - 1) + [3]

        # the main non-linear activation func
        act = self.get_activation(resmlp_act)

        # head
        self.input_dim = input_dim
        self.head = torch.nn.Sequential(*[torch.nn.Linear(input_dim, Ws[0]), act])

        # body
        body = []
        for i in range(1, D - 1):
            body += [torch.nn.Linear(Ws[i - 1], Ws[i]), act]

        # >>> new implementation of the body. Will replace the above
        if trial:
            inact = self.get_activation(resmlp_act)
            outact = self.get_activation(resmlp_act)
            if body_arch in ['resmlp']:
                n_block = (
                    D - 2
        ...
        # tail
        self.tail = torch.nn.Linear(
            input_dim, 3) if linear_tail else torch.nn.Sequential(
                *[torch.nn.Linear(Ws[D - 2], 3),
                  torch.nn.Sigmoid()])
    ...
    def forward(self, pts, viewdirs, features, step, total_freq_reg_step):
        ...
        indata = torch.cat(indata, dim=-1).to(torch.float32)

        indata = self.head(indata)
        indata = self.body(indata) + indata if self.use_residual else self.body(indata)
...
class TensorBase(torch.nn.Module):
    ...
    """def sample_ray_gau(self, rng, rays_o, rays_d, radii, N_samples=-1, is_train=True, ray_shape='cylinder', diag=True):
        near, far = torch.tensor(self.near_far[0]), torch.tensor(self.near_far[1])
        stepsize = self.stepSize
        ...
        rng = torch.arange(N_samples+1)[None].float()
        if is_train:
            rng = rng.repeat(rays_d.shape[-2],1)
            rng += torch.rand_like(rng[:,[0]])
        step = stepsize * rng.to(rays_o.device)
        ...
        mids = 0.5 * (interpx[..., 1:] + interpx[..., :-1])
        upper = torch.cat((mids, interpx[..., -1:]), dim=-1)
        lower = torch.cat((interpx[..., :1], mids), dim=-1)
        t_vals = lower + (upper - lower)  

        means, covs = self.cast_rays(
            t_vals, rays_o, rays_d, radii, ray_shape, diag=diag)

        rays_pts = rays_o[..., None, :] + rays_d[..., None, :] * mids[..., None]
        mask_outbbox = ((self.aabb[0] > rays_pts) | (rays_pts > self.aabb[1])).any(dim=-1)
        
        return (means, covs), mids, ~mask_outbbox"""
    ...
    def updateAlphaMask(self, gridSize=(200,200,200)):
        ...
        print(f"\nbbox: {xyz_min, xyz_max} alpha rest %%%f"%(total/total_voxels*100))
        return new_aabb
    ...
    def forward(self, rays_chunk, step=-1, total_freq_reg_step=15000, white_bg=True, is_train=False, ndc_ray=False, mip_sample=False, N_samples=-1):
