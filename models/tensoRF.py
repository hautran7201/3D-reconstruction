...
class TensorVM(TensorBase):
    ...
    def get_optparam_groups(self, lr_init_spatialxyz = 0.02, lr_init_network = 0.001):
        grad_vars = [ {'name': 'line_coef', 'params': self.line_coef, 'lr': lr_init_spatialxyz}, 
                      {'name': 'plane_coef', 'params': self.plane_coef, 'lr': lr_init_spatialxyz},
                      {'name': 'basis_mat', 'params': self.basis_mat.parameters(), 'lr':lr_init_network}]
        if isinstance(self.renderModule, torch.nn.Module):
            grad_vars += [{'name': 'renderModule', 'params':self.renderModule.parameters(), 'lr':lr_init_network}]
        return grad_vars
...
class TensorVMSplit(TensorBase):
    ...
    def get_optparam_groups(self, lr_init_spatialxyz = 0.02, lr_init_network = 0.001):
        grad_vars = [{'name': 'density_line', 'params': self.density_line, 'lr': lr_init_spatialxyz}, 
                     {'name': 'density_plane', 'params': self.density_plane, 'lr': lr_init_spatialxyz},
                     {'name': 'app_line', 'params': self.app_line, 'lr': lr_init_spatialxyz}, 
                     {'name': 'app_plane', 'params': self.app_plane, 'lr': lr_init_spatialxyz},
                     {'name': 'basis_mat','params': self.basis_mat.parameters(), 'lr':lr_init_network}]
        if isinstance(self.renderModule, torch.nn.Module):
            grad_vars += [{'name': 'renderModule', 'params':self.renderModule.parameters(), 'lr':lr_init_network}]
        return grad_vars
    ...
    def compute_appfeature(self, xyz_sampled):
        ...
        # return self.appModule((plane_coef_point * line_coef_point).T)
